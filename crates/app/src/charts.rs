...
pub fn elevation_polar(
    ...
    p.rect_filled(rect, 2.0, WELL);
    let c = rect.center();
    let r = rect.width().min(rect.height()) / 2.0 - 18.0;
    let floor = 30.0;
    let rad = |g: f64| (((g - peak) + floor) / floor).clamp(0.0, 1.0) as f32 * r;
    let at = |el: f64, g: f64| {
        let a = el.to_radians() as f32;
        c + Vec2::new(a.cos() * rad(g), -a.sin() * rad(g))
    };
    for db in [0.0, -10.0, -20.0] {
        let rr = rad(peak + db);
        p.circle_stroke(c, rr, Stroke::new(1.0, ETCH));
        p.text(
            c + Vec2::new(rr + 2.0, 10.0),
            Align2::LEFT_CENTER,
            format!("{db:.0}"),
            theme::figure(9.5),
            LEGEND,
    ...
    for el in (0..12).map(|i| i as f64 * 30.0) {
        let a = el.to_radians() as f32;
        p.line_segment([c, c + Vec2::new(a.cos(), -a.sin()) * r], Stroke::new(1.0, ETCH));
    }
    let pts: Vec<Pos2> = cut.iter().map(|&(el, g)| at(el, g)).collect();
    if pts.len() > 1 {
        p.add(Shape::line(pts, Stroke::new(2.0, TRACE)));
    }
    if let Some((el, g)) = mark {
        let a = el.to_radians() as f32;
        p.line_segment([c, c + Vec2::new(a.cos(), -a.sin()) * r], Stroke::new(1.0, READOUT));
        p.circle_filled(at(el, g), 4.0, READOUT);
    }
    p.text(
        Pos2::new(c.x - r - 4.0, c.y - 8.0),
        Align2::LEFT_BOTTOM,
        "behind",
        theme::legend_font(10.0),
        LEGEND,
